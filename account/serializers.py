...
class ActivationSerializer(serializers.Serializer):
    ...
    def validate_activation_code(self, activation_code):
        ...
        raise serializers.ValidationError({'message': 'Неправильно указан код активации'}, code=status.HTTP_400_BAD_REQUEST)
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate_username(self, username):
        ...
        if not user:
            raise serializers.ValidationError({'message': f'User with {username} username not found'}, code=status.HTTP_400_BAD_REQUEST)
        return username
